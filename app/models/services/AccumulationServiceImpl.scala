...
class AccumulationServiceImpl @Inject() (dao:AccumulationDAO) extends AccumulationService {
  ...
  def findOrCreateForToday(userId: UUID, gatheringId: Long, mantraId: Long): Future[Option[Accumulation]] = {
  	findForToday(userId, gatheringId, mantraId) flatMap { acc =>
  		acc match {
  		  case None =>
