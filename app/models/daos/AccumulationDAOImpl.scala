...
class AccumulationDAOImpl extends AccumulationDAO with DAOSlick {
  ...
  def save(accumulation: Accumulation): Future[Accumulation] = {
    ...
    val actions = (for {
      result <- (accumulationsTable returning accumulationsTable.map(_.id)).insertOrUpdate(dbAccumulationRow)
  ...
  def findForToday(userId: UUID, gatheringId: Long, mantraId: Long): Future[Accumulation] = {
    ...
    val query = accumulationsTable.filter { accumulation =>
      List(
  ...
  def counts(mantraId: Long)(maybeGatheringId: Option[Long] = None): Future[(Long, Long, Long, Long)] = {
    ...
    val q = if (maybeGatheringId == None) accumulationsTable.filter(_.mantraId === mantraId) else accumulationsTable.filter(_.mantraId === mantraId).filter(_.gatheringId === maybeGatheringId.get)
